...
public class GameAIServerUDP extends GameConnectionServer<UUID>{
    ...
    public void sendNPCinfo() {
        try {
            ...
            message += "," + (npcCtrl.getNPC()).getY();
            message += "," + (npcCtrl.getNPC()).getZ();
            message += "," + (npcCtrl.getNPC()).getRotation();
            sendPacketToAll(message);
        } catch (IOException e) {
            System.out.println("couldn't send msg");
            e.printStackTrace();
    ...
    public void sendNPCstart(UUID clientID) {
        try {
            String message = new String("npcstart");
            message += "," + (npcCtrl.getNPC()).getX();
            message += "," + (npcCtrl.getNPC()).getY();
            message += "," + (npcCtrl.getNPC()).getZ();
            message += "," + (npcCtrl.getCriteria());
	...
	public void processPacket(Object o, InetAddress senderIP, int port)
	{
		...
		{	// JOIN -- Case where client just joined the server
			...
			// CREATE_NPC -- Case where server receives a create message (to specify NPC location)
			// Received Message Format: (createNPC,localId,x,y,z)
			...
			// Case where sever receives notive that an av is close to the NPC
			...
			// Case where server receives request for NPC info
			// Received Message Format: (npcinfo, id)
    ...
    // Informs clients of the whereabouts of the NPC
	...
	public void sendMoveNPCMsg(UUID npcID, String[] position) {
		try {
			String message = "mnpc," + npcID.toString();
			message += "," + position[0];
			message += "," + position[1];
			message += "," + position[2];
			forwardPacketToAll(message, npcID);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
